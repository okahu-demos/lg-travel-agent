...
# Shared instruction that makes every assistant ask a follow-up question when the
# user's request is missing a required detail, instead of guessing or ignoring it.
ASK_FOLLOW_UP = (
    " If the user's request is missing a required detail, do NOT guess or invent it "
    "and do NOT call your tool. Instead, ask the user a concise follow-up question for "
    "exactly the missing information. Only call your tool once you have every required detail."
)
...
async def setup_agents():
    ...
    model=model_factory(),
        ...
        system_prompt=(
            ...
            "A flight booking requires both a departure airport and an arrival airport."
            + ASK_FOLLOW_UP
        ),
    ...
    model=model_factory(),
        ...
        system_prompt=(
            ...
            "A hotel booking requires the hotel name. Book the hotel only if the user explicitly asks."
            + ASK_FOLLOW_UP
        ),
    ...
    model=model_factory(),
        tools=weather_tools,
        system_prompt=(
            "You are a weather information assistant. Please use the tool available to you for "
            "checking weather. A weather lookup requires a city name; extract it from the user query "
            "and pass it to the weather tool."
            + ASK_FOLLOW_UP
        ),
    ...
    model=model_factory(),
        system_prompt=(
            ...
            "If an assistant responds with a follow-up question because the user's request was "
            "incomplete, relay that question back to the user verbatim and wait for their answer "
            "in the next turn. Do not invent missing details yourself."
        )
...
async def run_agent_session(session_id: str):
    ...
    while True:
        try:
            request: str = input("\nI am a travel booking agent. How can I assist you with your travel plans? (You can ask me to book flights, hotels, or check the weather at any location. Type 'exit', 'quit', or 'bye' to leave.): \n")
        except EOFError:
            print("\nBye...")
            break
